		...
		<div class="wrap postbox" style="padding: 10px;">	  			
			<form name="emba_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
				...
				<table cellpadding="5" cellspacing="5">
					...
					<? if ($display == 'custom') { ?>
					<tr><td>Button Code</td><td><p>Copy and paste this button code anywhere you want it to appear...</p><?=$custom_button?></td></tr>					
					<? } ?>
...
<div style="border-bottom: 1px solid #555; padding-bottom: 10px; margin-bottom: 10px; margin-right: 25px;">
	<div class="wrap">
		<span style="float: right; display: block;"><form method="post"><input type="text" name="q" size="40" value="<?=htmlspecialchars($_REQUEST['q'])?>"><input type="submit" name="s" value="Search" class="button button-secondary button-normal"></form></span>
		<h2>Embed Articles Archives</h2>
		<? if (!empty($pub_value) && isset($pub_value)) { ?>
		<p>
			<div id="archives">
				<ul>
					<li><a href="#archives">Curated Embedables</a></li>
					<li><a href="#community">The Community</a></li>
					<li><a href="http://embedarticles.com/api/index.php?p=embeds&k=<?=$pub_value?>">Embed Reports</a></li>
				</ul>
				<div id="archives">
					<?
						$_archives = file_get_contents('http://embedarticles.com/api/?_page=' . $page . '&p=archives&k=' . $pub_value . '&q=' . urlencode($_REQUEST['q']));
						print $_archives;
					?>
				</div>
				<div id="community">
